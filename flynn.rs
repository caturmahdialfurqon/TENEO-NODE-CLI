...
async fn get_local_storage() -> Result<Value, Box<dyn std::error::Error>> {
    let data = fs::read_to_string("localStorage.json").unwrap_or_else(|_| "{}".to_string());
    Ok(serde_json::from_str(&data)?)
}

async fn set_local_storage(data: Value) -> Result<(), Box<dyn std::error::Error>> {
    let current_data = get_local_storage().await?;
    let new_data = json!({
        ...current_data,
        ...data
    });
    fs::write("localStorage.json", serde_json::to_string(&new_data)?)?;
    Ok(())
}

async fn connect_websocket(user_id: &str, app_state: Arc<Mutex<AppState>>) -> Result<(), Box<dyn std::error::Error>> {
    let version = "v0.2";
    ...
    let connection_time = SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs();
    set_local_storage(json!({"lastUpdated": connection_time})).await?;
...
async fn start_pinging(app_state: Arc<Mutex<AppState>>) {
    ...
    let ping_task = tokio::spawn(async move {
        loop {
            ...
            if let Some(ref mut socket) = app_state.socket {
                ...
                let last_ping_date = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
                set_local_storage(json!({"lastPingDate": last_ping_date})).await.unwrap();
            }
        }
    });
...
async fn start_countdown_and_points(app_state: Arc<Mutex<AppState>>) {
...
async fn update_countdown_and_points(app_state: Arc<Mutex<AppState>>) {
    let local_storage = get_local_storage().await.unwrap();
    if let Some(last_updated) = local_storage["lastUpdated"].as_i64() {
        ...
        if diff > 0 {
            ...
            if rand::thread_rng().gen_bool(0.1) {
                let bonus = rand::thread_rng().gen_range(0.0..2.0);
    ...
    set_local_storage(json!({
        "potentialPoints": app_state.potential_points,
        "countdown": app_state.countdown
    })).await.unwrap();
}

async fn get_user_id() -> Result<String, Box<dyn std::error::Error>> {
    ...
    let user_id = data["user"]["id"].as_str().unwrap().to_string();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let local_storage_data = get_local_storage().await?;
    let mut user_id = local_storage_data["userId"].as_str().map(|s| s.to_string());
    ...
    if user_id.is_none() {
        ...
        user_id = Some(input.trim().to_string());
        set_local_storage(json!({"userId": user_id})).await?;
    }

    println!("Menu:\n1. Logout\n2. Start Running Node\n3. Get User ID\nChoose an option: ");
    let mut option = String::new();
    io::stdin().lock().read_line(&mut option)?;

    match option.trim() {
        "1" => {
            set_local_storage(json!({})).await?;
            println!("Logged out successfully.");
        },
        "2" => {
            if let Some(user_id) = user_id {
                start_countdown_and_points(Arc::clone(&app_state)).await;
                connect_websocket(&user_id, Arc::clone(&app_state)).await?;
            } else {
                println!("User ID not found. Please log in first.");
            }
        },
        "3" => {
            let new_user_id = get_user_id().await?;
            start_countdown_and_points(Arc::clone(&app_state)).await;
            connect_websocket(&new_user_id, Arc::clone(&app_state)).await?;
        },
