...
def start_countdown_and_points
  $countdown_interval.kill if $countdown_interval
...
def update_countdown_and_points
  ...
  if last_updated
    ...
    if diff > 0
      ...
      max_points = 25
      time_elapsed = (now - Time.parse(last_updated)).to_f
...
def main
  ...
  if !user_id
    puts "User ID not found. Would you like to:"
    puts "1. Login to your account"
    puts "2. Enter User ID manually"
    ...
    puts "Menu:"
    puts "1. Logout"
    puts "2. Start Running Node"
    print "Choose an option: "
    option = gets.chomp

    case option
    when '1'
      File.delete('localStorage.json') if File.exist?('localStorage.json')
